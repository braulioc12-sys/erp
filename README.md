...
**Mantenimiento → Materiales** (pedido de Braulio, 28 ago 3ª ronda) es un catálogo de materiales (nombre + costo unitario) que se puede marcar al crear una orden o agregar después desde su detalle, indicando la cantidad usada — el subtotal de cada material (cantidad × costo unitario) se suma al costo sugerido de la orden junto con la mano de obra. El detalle de la orden muestra por separado el costo de materiales y el total de la orden. Igual que con los trabajos, cada orden guarda su propia copia del nombre y costo unitario del material al momento de agregarlo, así que editar o desactivar un material en el catálogo después no cambia órdenes ya registradas. **Viene con una lista de ejemplo (filtro de aceite, aceite de motor, pastillas de freno, etc. con precios de referencia) — AJUSTAR: reemplázala por la lista real de materiales y precios de tu taller en Mantenimiento → Materiales** (o edita `DEFAULT_MATERIALS` en `app/seed_data.py` si prefieres cargarla desde código). Mismo permiso que el resto de Mantenimiento: Administrador y Operador la ven, solo Administrador agrega/desactiva materiales.
