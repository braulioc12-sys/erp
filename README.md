...
| Inventarios (repuestos, proveedores, compras) | Ver/Crear/Editar | Solo ver |
...
- **Repuestos** (Inventarios, pantalla principal): catálogo de repuestos con su costo unitario de referencia y su **stock actual** en almacén. El costo unitario se actualiza solo al precio de la última compra recibida (ver más abajo); el stock se puede además ajustar a mano (para corregir un conteo físico) sin pasar por una compra.
...
- **Compras** (Inventarios → Compras → + Nueva compra): registras el proveedor, el N° de orden de compra, la fecha, y uno o más repuestos con su cantidad y precio unitario en esa compra (una compra puede incluir varios repuestos distintos). **La compra nace como "Pendiente" y todavía no afecta el stock** — recién al marcarla **"✓ Marcar como recibida"** desde su detalle (cuando el pedido realmente llega al almacén) se suma la cantidad de cada línea al stock del repuesto correspondiente, y su costo unitario de referencia se actualiza al precio pagado en esa compra (último precio gana). Una compra pendiente se puede eliminar; una ya recibida no (ya afectó el stock, borrarla dejaría el historial inconsistente).
