...
21 sep, pedido de Braulio: "que aparte de facturar los viajes, tambien se puedan emitir facturas no relacionadas a viajes, como por ejemplo alquileres... de todo tipo". Facturación → "Generar factura" ya no exige marcar al menos un viaje — al elegir un cliente aparece también una sección **"Ítems adicionales"** donde se puede agregar cualquier línea libre (descripción + monto), con un botón "+ Agregar línea" para sumar cuantas haga falta. Se pueden combinar viajes marcados + ítems adicionales en la misma factura, o dejar los viajes sin marcar y facturar solo con ítems adicionales (por ejemplo, un alquiler de equipo a un cliente que no tiene ningún viaje pendiente).

Cuando NO se marca ningún viaje, aparece un selector **"Empresa que emite"** (Harraso/BRMS) — si se marca al menos un viaje, la empresa se toma automáticamente de esos viajes, igual que siempre (el selector queda deshabilitado en ese caso).

**Detracción (SPOT) — importante:** el 4% de detracción que el sistema calcula automáticamente (código "027", específico de transporte de carga) solo se aplica cuando la factura es 100% de viajes, exactamente igual que antes de este cambio. Si la factura incluye algún ítem adicional (aunque también tenga viajes), el sistema NO le aplica ninguna detracción automática — un alquiler u otro servicio distinto de transporte de carga tendría, en caso de corresponder, su propio código de detracción (no confirmado). El sistema avisa con un mensaje al generar una factura así por encima de S/400; revísalo con tu contador o desde el propio portal de tefacturo.pe (tiene un toggle "Activar detracción" al emitir a mano) antes de enviarla a SUNAT.
