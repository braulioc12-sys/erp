...
## Menú alfabético, y Catálogos → Conceptos de detracción (22 sep)

Pedido de Braulio: *"ordenemos este menu por orden alfabetico dejando simplemente panel como primero"*. El menú lateral (`app/templates/base.html`) ya no sigue el orden en que se fueron agregando los módulos — está ordenado alfabéticamente por la **etiqueta** que ve el usuario (Catálogos, Clientes, Conductores, Cotizaciones, Facturación...), con **Panel** fijo arriba de todos. Si en el futuro se agrega un módulo nuevo, su link va en el lugar alfabético que le corresponda, no al final de la lista.

**Catálogos → Conceptos de detracción.** Pedido de Braulio: *"en la parte de catalogos hay que incluir conceptos de detraccion y en este se puedan agregar o modificar los conceptos o porcentajes de los que ya tienes registrados para las facturas"*. La lista de bienes/servicios sujetos a detracción (código SUNAT, nombre, porcentaje) que hasta ahora estaba fija en el código (ver la sección "Empresa primero, y detracción por catálogo" más abajo) ahora vive en una tabla editable, con su propia pantalla en **Catálogos → Conceptos de detracción**. A diferencia de otros catálogos como Grifos o Bancos (que solo se activan/desactivan), acá **cada fila es directamente editable** — código, nombre y porcentaje — porque Braulio pidió explícitamente poder "modificar los conceptos o porcentajes", no solo darlos de baja. También se pueden agregar conceptos nuevos.

El código **"027" (transporte y/o traslado de bienes)** es, además, el que usa el sistema para el cálculo **automático** de detracción en una factura 100% de viajes (ver `compute_detraction()` en `app/helpers.py`) — si le cambias el porcentaje desde esta pantalla, ese cálculo automático usa el nuevo valor desde la próxima factura que se genere, sin tocar código. Si llegaras a desactivar el "027" por error, el cálculo automático no se rompe: cae de vuelta al 4% que traía por defecto (documentado en el propio código), aunque en ese caso deja de reflejar cambios de porcentaje hasta que lo reactives.

La tabla se siembra **una sola vez**, con los mismos 15 códigos/porcentajes que el sistema ya venía usando (los que dio Braulio, con las mismas siete posibles discrepancias contra fuentes públicas ya documentadas más abajo) — desactivar, editar o agregar conceptos desde esta pantalla es completamente independiente de esa siembra inicial y no se vuelve a sobreescribir en despliegues futuros.

## Jornada laboral (antes "Descansos laborales") (22 sep)

22 sep, 2da ronda, pedido de Braulio ("el menu 'descansos' hay que renombrarlo por 'jornada laboral'"): el menú y el título de la pantalla ahora dicen **"Jornada laboral"** — es el mismo módulo de siempre (mismas rutas, mismo permiso `descansos` en Usuarios, misma tabla `driver_rests`), solo cambió la etiqueta que ve el usuario, igual que pasó antes con "Ubicación GPS" (antes "Integraciones"). También se agregó, arriba de todo en la pantalla, un **buscador por nombre de conductor** ("Buscar conductor" + botón "Buscar") que filtra la tabla "Estado de conductores" — antes el único filtro por conductor estaba más abajo, en el historial de descansos ya registrados (ese selector se mantiene, es un filtro aparte y más preciso para ver el historial de uno solo).
...
### Rediseño del formulario "Registrar gasto" (22 sep)

Pedido de Braulio: *"el viaje y la unidad que queden arriba. Luego que salga la parte contable, Tipo de comprobante, Monto, Concepto, Nro de cuenta, Nro de documento, Ruc proveedor, Razon social... la parte de descripcion que salga al final del llenado y sea opcional"*. El formulario de gasto (tanto al crear como al editar) quedó reordenado en ese mismo orden: Viaje/Unidad y Fecha arriba, luego la sección **"Datos contables"** con Tipo de comprobante → Monto → Concepto (+ N° de cuenta, que se sigue completando solo al elegirlo) → N° de documento → RUC/Razón social del proveedor, y **Descripción** (opcional, sin cambios en el backend) al final de todo el formulario, justo antes de adjuntar el comprobante — [nota: la Descripción quedó como el último campo de texto; el adjuntar la foto/PDF del comprobante sigue inmediatamente antes por ser, en la práctica, parte de "cerrar" el registro del gasto].

**Moneda y Tipo de cambio ya no se muestran ni se piden en este formulario** (pedido explícito: "la moneda siempre sera soles por lo tanto eso no es necesario mostrar ni tampoco el tipo de cambio, que simplemente salga a la hora de exportar el excel"). La moneda sigue guardándose como "S" (soles) por defecto — como ya hacía el sistema cuando el campo se dejaba en blanco — y el tipo de cambio se sigue completando automáticamente con el de SUNAT del día del gasto exactamente igual que antes (ver más abajo); solo se quitó del formulario la posibilidad de forzarlo a mano, porque ambos valores ahora solo importan al exportar el Resumen contable / Historial a Excel, no en la pantalla de registro. Si algún día hiciera falta corregir un tipo de cambio puntual, por ahora habría que hacerlo directo en la base de datos — no hay ninguna pantalla para eso.
...
**Camino 2 — A través de un OSE (Operador de Servicios Electrónicos), en este caso tefacturo.pe.** Le mandas un JSON simple por HTTPS con los datos del comprobante, el OSE arma el XML, lo firma con su propio certificado, lo envía a SUNAT y te devuelve el PDF, el XML y la constancia de aceptación (CDR). Es el camino que usa casi cualquier negocio pequeño o mediano, y **es el que implementa este sistema** (`app/integrations/sunat_ose.py`), contra la API real de tefacturo.pe (`https://api.tefacturo.pe/doc/integracion/`, confirmada 7 sep — ver "Aviso importante" más abajo). tefacturo.pe cobra por comprobante emitido (consulta tarifas directamente con ellos) — es un costo aparte del hosting del ERP.
...
1. Crea una cuenta en tefacturo.pe para **cada empresa que vaya a emitir** — primero una de pruebas en [api.tefacturo.pe/doc/integracion/registro](https://api.tefacturo.pe/doc/integracion/registro/), y para producción pídesela directamente a tefacturo.pe. El login real es por RUC + correo + clave (no una "ruta"/"token" genéricos) — cada empresa necesita su propia cuenta.
...
5. Al generar una guía (Viajes → viaje en curso/entregado → "Generar guía de remisión"), completa el **ubigeo de partida y de llegada** (código INEI de 6 dígitos del distrito) — SUNAT lo exige y el sistema no tiene un catálogo propio para autocompletarlo.
6. Emite una factura de prueba (Facturación → detalle de una factura → "Enviar a SUNAT") o una guía (Guías → detalle → "Enviar a SUNAT") y revisa la respuesta. Si se acepta, el sistema descarga y guarda automáticamente el PDF real que devuelve tefacturo.pe (link "PDF" en el detalle).

### Empresa primero, y detracción por catálogo (22 sep)

Pedido de Braulio: *"en facturacion la primera pantalla debe ser elegir Harraso o BRMS. Luego cuando se genera factura ya no debe salir ese campo de empresa que emite"*. Igual que en Viajes y Liquidaciones, **Facturación** ahora pide elegir la empresa (Harraso o BRMS) como primera pantalla, tanto para ver la lista de facturas como para "Generar factura" — un link "↺ Cambiar de empresa" permite volver a elegir. Con la empresa ya fija, "Generar factura" **ya no tiene el `<select>` "Empresa que emite"**: la empresa queda fija en un campo oculto durante todo el formulario, y "Viajes entregados pendientes de facturar" solo trae los de esa empresa (antes se mezclaban los de ambas con un aviso de "factúralos por separado").

**Detracción — solo Harraso, nunca BRMS.** Pedido explícito de Braulio: *"recuerda que solo Harraso emite con detraccion, BRMS no"*. Para una factura de BRMS la sección de detracción no existe en absoluto — ni el 4% automático (código "027", cuando la factura es 100% de viajes) ni la confirmación manual (ítems adicionales); tampoco se puede aplicar después desde el detalle. Para Harraso, el cuadro de detracción en "Generar factura" quedó compacto: por defecto solo se ve un checkbox **"Afecta a detracción"**, y el resto de campos (bien, código, porcentaje, monto, cuenta) aparece recién al marcarlo — solo tiene sentido cuando la factura tiene algún ítem adicional; en una factura 100% de viajes el 4%/"027" se sigue calculando solo, como siempre.

El código ya no se escribe a mano: se elige el **bien** de una lista (los "Códigos Principales de Bienes" que dio Braulio del catálogo de SUNAT) y el código/porcentaje se completan solos, de solo lectura — mismo selector en el detalle de una factura ("Editar / quitar detracción"). **Ojo:** al verificar esa lista contra fuentes públicas actuales (docs.factpro.la, estudiobonilla.pe) aparecieron diferencias en varios códigos — "003 Alcohol etílico", "008 Madera", "014 Carnes y despojos comestibles" y "017 Harina/pellets de pescado" figuran ahí como 4% (acá quedaron con el % que dio Braulio), y "006 Algodón", "013 Animales vivos" y "015 Abonos/cueros/pieles" aparecen en al menos una fuente como códigos ya derogados desde ~2014. No se corrigieron a propósito (ver el comentario junto a `DETRACTION_GOODS_CATALOG` en `app/helpers.py`) — **confirma con tu contador antes de usar alguno de esos siete códigos en una factura real**, un depósito de detracción con el porcentaje o código equivocado no se puede corregir después con SUNAT.

### Detracción: monto automático al elegir el bien, y aviso sobre el envío real a SUNAT (22 sep)

Braulio mandó, además, una factura real de Harraso ya emitida (FFA1-1176, tomada del portal de tefacturo.pe) que sí muestra en el PDF una tabla "Concepto de Detracción" (Código del Bien, Monto, Porcentaje, Cuenta Bco. de la Nación) — y reportó que las facturas que emite este sistema no la muestran. Dos cosas separadas:

1. **Monto que se calcula solo, apenas eliges el bien (arreglado).** Antes había que dejar el campo "Monto detraído" en blanco para que el servidor lo calculara recién al guardar la factura. Ahora, tanto en "Generar factura" como al editar la detracción desde el detalle de una factura, el monto se calcula y se muestra al instante en pantalla apenas eliges un bien de la lista (o cambia el total de ítems adicionales) — sigue siendo editable a mano por si tu contador te da un monto distinto; una vez que lo editas tú mismo, ya no se pisa con el cálculo automático.
2. **El envío real a SUNAT todavía NO incluye la detracción (sin resolver, no es nuevo).** Esto ya estaba documentado como pendiente desde antes (ver el aviso "⚠ Pendiente" en el detalle de cada factura) — se volvió a revisar a fondo la documentación pública de tefacturo.pe el 22 sep (el endpoint de factura, sus catálogos, la página de "Ejemplos JSON") y, igual que en la revisión anterior, no aparece ningún campo documentado para reportar detracción. Su portal web sí tiene un toggle "Activar detracción" al emitir a mano — así que su sistema sí lo soporta, solo que el nombre del campo para la API REST no está en su documentación pública. A propósito **no se adivinó un nombre de campo** para este envío (a diferencia de otros casos de esta integración donde sí había una pista real cerca del campo faltante): equivocarse en un dato de detracción es peor que dejarlo pendiente con el aviso bien visible. **Braulio: la próxima vez que hables con soporte técnico de tefacturo.pe (el mismo contacto que ya usaste para otras dudas de esta integración), pregúntales puntualmente qué campo del JSON hay que mandar al endpoint de factura para reportar detracción (código de bien, %, monto, cuenta del Banco de la Nación) — en cuanto tengas la respuesta, avísame y lo agrego.** Mientras tanto, para una factura de Harraso con detracción, regístrala también desde el portal de tefacturo.pe (o pídeles que se la agreguen ellos con el número de comprobante), para que el PDF real que ve el cliente sí la muestre.

### Detracción: tefacturo.pe confirmó el formato, ya se envía a SUNAT (falta un dato por concepto) (23 sep)

Braulio consiguió de tefacturo.pe un JSON real de ejemplo (una factura de OTRO cliente de ellos, con detracción de "Azúcar") que por fin confirma el campo que llevábamos semanas sin encontrar en su documentación pública: un bloque `detraccion` suelto en la raíz del payload de factura, con `codigoBienServicio`, `numeroCuenta`, `porcentaje` y `redondeo`.

**Lo que ya funciona solo, sin configurar nada:** `numeroCuenta` (la cuenta del Banco de la Nación) y `porcentaje` salen directo de la factura — apenas Braulio confirme la detracción de una factura de Harraso, esos dos datos ya viajan correctos a SUNAT.

**Lo único que falta, y por qué:** el campo `codigoBienServicio` del ejemplo trae `"AZUCAR"` — una palabra clave propia del sistema de tefacturo.pe (el nombre del bien, en mayúsculas), **no** el código numérico de SUNAT que usamos acá ("027", etc.). No hay forma de adivinar la palabra clave que le corresponde a "transporte de bienes por vía terrestre" (ni a ningún otro concepto del catálogo) sin que tefacturo.pe la confirme uno por uno — así que se agregó una columna nueva y editable, **"Código en tefacturo.pe"**, en Catálogos → Conceptos de detracción. Mientras un concepto no tenga esa palabra clave cargada, sus facturas se siguen emitiendo con total normalidad, solo que sin reportar la detracción a SUNAT (igual que hasta ahora) — el aviso en el detalle de la factura lo deja claro y enlaza directo a Catálogos para cargarlo. En cuanto se carga, **todas** las facturas (pasadas y futuras) que usen ese mismo concepto empiezan a reportar la detracción completa desde el próximo "Enviar a SUNAT".

**Siguiente paso para Braulio:** pídele a tefacturo.pe la palabra clave exacta de `codigoBienServicio` para "027 — Transporte y/o traslado de bienes" (el único código que usa hoy), y cárgala en Catálogos → Conceptos de detracción → fila "027" → columna "Código en tefacturo.pe". Si en el futuro se usa otro concepto del catálogo, pide también su palabra clave antes de confiar en que esa factura reporte la detracción a SUNAT.

**Actualización (23 sep, 2da ronda) — resuelto, ya no hace falta el paso anterior.** Jorge, de soporte de tefacturo.pe, confirmó por WhatsApp la palabra clave completa (les compartió su "Catálogo Tipo de Detracción" entero, código SUNAT → palabra clave, para los ~40 conceptos que manejan): el 027 (transporte de bienes) usa `TRANSPORTE_DE_BIENES`, al 4% — coincide con lo que ya teníamos configurado. Como ahora es un dato confirmado por la fuente oficial (no una palabra a adivinar), se agregó un autocompletado en `app/db.py` (`_TEFACTURO_CODIGO_BIEN_SERVICIO` / `_backfill_tefacturo_codigo_bien_servicio_*`) que carga solo, en cada arranque del servidor, la palabra clave de tefacturo.pe para **todos** los conceptos del catálogo de Braulio que ya están en esa lista confirmada (14 en total) — sin pisar nunca un valor que Braulio ya haya cargado o corregido a mano desde Catálogos. En la práctica: **el 027 (el único que usa Harraso hoy) queda funcionando solo, sin que Braulio tenga que tocar nada en Catálogos.**

**Ojo con esto — pendiente de confirmar con el contador, no se tocó solo.** El catálogo completo de tefacturo.pe trae, para 5 conceptos que Braulio tiene cargados mayormente de forma referencial (no los usa hoy: "008 Madera", "013 Animales vivos", "014 Carnes y despojos comestibles", "015 Abonos/cueros/pieles", "017 Harina/pellets de pescado"), un porcentaje **distinto** al que quedó configurado en el sistema (todos al 4% según tefacturo.pe, contra 12% o 10% que tiene hoy Catálogos) — coincide con lo que ya advertían las fuentes públicas que se habían revisado antes (ver la nota junto a `_DETRACTION_GOODS_SEED` en `app/helpers.py`). No se corrigió el porcentaje solo porque un depósito de detracción con el % equivocado no se puede arreglar después con SUNAT — si Braulio confirma con su contador que el de tefacturo.pe es el correcto para alguno de estos, se corrige a mano desde Catálogos → Conceptos de detracción. El "006 Algodón" tampoco aparece en el catálogo de tefacturo.pe, lo que refuerza que ya no está vigente.

### Facturas sin viaje (ítems adicionales — alquileres, etc.)

21 sep, pedido de Braulio: "que aparte de facturar los viajes, tambien se puedan emitir facturas no relacionadas a viajes, como por ejemplo alquileres... de todo tipo". Facturación → "Generar factura" ya no exige marcar al menos un viaje — al elegir un cliente aparece también una sección **"Ítems adicionales"** donde se puede agregar cualquier línea libre (cantidad + descripción + precio unitario), con un botón "+ Agregar línea" para sumar cuantas haga falta. Se pueden combinar viajes marcados + ítems adicionales en la misma factura, o dejar los viajes sin marcar y facturar solo con ítems adicionales (por ejemplo, un alquiler de equipo a un cliente que no tiene ningún viaje pendiente).

**Detracción (SPOT) — importante:** el 4% de detracción que el sistema calcula automáticamente (código "027", específico de transporte de carga) solo se aplica cuando la factura es 100% de viajes, exactamente igual que antes de este cambio, y solo para Harraso (ver arriba). Si la factura incluye algún ítem adicional (aunque también tenga viajes), el sistema NO le aplica ninguna detracción automática — se confirma a mano eligiendo el bien correcto de la lista. El sistema avisa con un mensaje al generar una factura de Harraso así por encima de S/400 sin detracción confirmada; revísalo con tu contador antes de enviarla a SUNAT.

### Facturar a un cliente que todavía no está registrado

22 sep, pedido de Braulio: "en este menu, debe haber la opcion de generar cliente nuevo y se pueda emitir una factura de cliente que no este registrado". En Facturación → "Generar factura", junto al selector de cliente hay un botón **"+ Registrar cliente nuevo"** que despliega un formulario corto (nombre/razón social, RUC, dirección, teléfono, correo) — al guardarlo, crea el cliente y vuelve a la misma pantalla con ese cliente ya elegido, listo para facturarle (con viajes si los tuviera, o con ítems adicionales si es un cliente sin ningún viaje todavía). El RUC tiene autocompletado desde SUNAT, igual que en Cotizaciones.

Este atajo queda disponible con el mismo permiso de Facturación (no el de Clientes) a propósito: Contabilidad —el rol que de verdad usa Facturación día a día— solo tiene acceso de *lectura* al módulo Clientes, así que sin este atajo no podría dar de alta un cliente nuevo por su cuenta para facturarle. El cliente creado así queda igual de completo que uno creado desde Clientes (aparece ahí también, se puede editar después).

### Aviso importante — léelo antes de emitir comprobantes reales

Esta integración se construyó (7 sep, segunda ronda) contra la **documentación técnica real de tefacturo.pe** (`https://api.tefacturo.pe/doc/integracion/`, compartida por Braulio) — a diferencia de un primer intento el mismo día basado en un manual suelto de 2019 que no documentaba autenticación ni guía de remisión. Con la documentación real confirmada, `app/integrations/sunat_ose.py` implementa: el login (RUC + correo + clave → token JWT de tefacturo.pe), la emisión de facturas, la emisión de **guías de remisión — modalidad Transportista** (Harraso/BRMS es quien transporta la carga, no el dueño de la mercadería), y la descarga automática del PDF real de cada comprobante emitido.

Aun así, **no se ha podido probar contra una cuenta real** (esta instalación no tiene usuario/clave de tefacturo.pe) — antes de emitir un comprobante real, ten en cuenta:

- **Guía remitente no implementada a propósito**: tefacturo.pe también documenta un endpoint de guía "Remitente" — no se implementó porque esa guía la debe emitir el DUEÑO de la carga (el cliente de Harraso, con su propia cuenta), no la empresa de transporte. Avisar si tu operación necesita que Harraso/BRMS también la emita.
- **`destinatario` = mismo dato que `remitente`** en la guía: el sistema solo conoce un cliente por viaje, así que usa el mismo para quien envía y quien recibe la carga. Si en la práctica suelen ser empresas distintas, hay que agregar un campo aparte.
- **Ubigeo manual**: no hay catálogo de ubigeos en el sistema — se escribe a mano en el formulario de la guía.
- **Registro MTC**: campo nuevo y obligatorio para la guía transportista, todavía sin completar (`HARRASO_MTC_REGISTRATION`/`BRMS_MTC_REGISTRATION`).
- **XML/CDR no se descargan todavía** — solo el PDF (tefacturo.pe también documenta `consultarXml`/`consultarCdr`, quedan pendientes si hicieran falta).

**Antes de emitir un solo comprobante a un cliente real, pide a tu contador (o a alguien con experiencia en facturación electrónica peruana) que revise los primeros comprobantes emitidos en modo de pruebas.** Mientras tanto, el resto del sistema (facturas y guías internas, control de estados, historial) funciona con normalidad sin esta integración; simplemente los comprobantes quedan como "No enviados" hasta que la actives y confirmes.
...
- **Datos bancarios y de la empresa (Harraso):** el RUC, dirección, correo, teléfono y las cuentas bancarias (Banco de la Nación, BCP ahorro y cuenta corriente) que aparecen en el PDF cuando la empresa es Harraso vienen de variables de entorno (`COMPANY_RUC`, `COMPANY_ADDRESS`, `COMPANY_EMAIL`, `COMPANY_PHONE`, `COMPANY_BANK_*` — ver `.env.example`), precargadas con los datos reales de Harraso tomados de una cotización de referencia. **AJUSTAR:** antes de enviar cotizaciones reales a clientes, confirma que esos números de cuenta y datos de contacto sigan vigentes — si algo cambió, se actualiza sin tocar código, solo la variable de entorno correspondiente. Para los datos de BRMS, ver el punto "Empresa que cotiza" arriba.
