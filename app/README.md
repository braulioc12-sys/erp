...
- **Catálogos** (solo Administrador): agrega o desactiva conceptos de mantenimiento e ítems de inspección desde la web, sin tocar código (los conceptos de gasto, con su cuenta contable, se administran aparte en Liquidaciones → Conceptos).
...
**El costo se calcula solo, pero se puede ajustar**: al marcar un trabajo, además se elige qué **tipo de mecánico** lo va a hacer (Senior/Junior/Practicante/Otros — ver más abajo), y el campo "Costo (S/)" se precarga automáticamente sumando, por cada trabajo marcado, sus minutos × el costo por minuto de ese tipo (configurable en Catálogos) — se puede editar a mano después si hace falta sumar repuestos u otros gastos que no sean mano de obra.
...
### Orden de mantenimiento: marcar trabajos terminados/pendientes, tipo de mecánico y asignar mecánico
